...
- [프로젝트 구조](#프로젝트-구조)
- [ERD](#erd)
- [데이터 모델 (VO 기준)](#데이터-모델-vo-기준)
- [라우팅 개요](#라우팅-개요)
  - [라우팅 구조 (Front Controller)](#라우팅-구조-front-controller)
  - [사용자 (User)](#사용자-user)
  - [강의 (Class)](#강의-class)
  - [공지 (Notice)](#공지-notice)
  - [자료실 (Library)](#자료실-library)
  - [Q&A & 댓글](#qa--댓글)
  - [통합 검색](#통합-검색)
  - [마이페이지](#마이페이지)
- [출결 (Attendance) 모듈 — 자세한 흐름](#출결-attendance-모듈--자세한-흐름)
  - [1) 출결 리스트 (강사·학생 공통 진입)](#1-출결-리스트-강사학생-공통-진입)
  - [2) 수강 중 강의 (학생용 정보 카드)](#2-수강-중-강의-학생용-정보-카드)
  - [3) 출결 관리 (강사용, 특정 날짜의 명단·상태)](#3-출결-관리-강사용-특정-날짜의-명단상태)
  - [4) 인증코드 발급 (강사용)](#4-인증코드-발급-강사용)
  - [5) 출석체크 (학생용, 인증코드 기반)](#5-출석체크-학생용-인증코드-기반)
  - [6) 출석정보 (개인 요약 캘린더)](#6-출석정보-개인-요약-캘린더)
- [권한별 기능 요약](#권한별-기능-요약)
  
---

## 프로젝트 개요
- **기획의도**: 수기로 진행되던 출결을 간단한 인증으로 전환해 행정 업무를 줄이고 편의성·수업 효율을 개선
- **프로젝트 목적**: JAVA/HTML/DB 역량 강화와 MVC 패턴 이해도 제고

핵심 목적은 다음과 같습니다.
- **수업 운영: 강의 등록·수정·삭제, 수강 신청 관리, 수강생 목록·현황 제공**
- **출결 관리:**
  - 학생: 인증코드 기반 실시간 출석체크, 개인 출결 캘린더 조회
  - 강사: 강의별 출결 현황 조회/수정, 인증코드 발급(6자리)
- **커뮤니케이션: 공지사항, 자료실(파일 업로드), Q&A(학생 질문/강사 답변)**
- **역할 기반 권한: 관리자(A) / 강사(T) / 학생(S) 로 구분하여 화면·행동을 제어**
- **안정적인 구조: Front Controller로 라우팅을 일원화하고, JSP/JDBC로 구성해 의존성 최소화와 학습/유지보수 용이성을 확보**
...
| 구분 | 기간 | 주요 내용 |
|:--:|:--:|:--|
| 1 | ~ 24.10.14(월) | **주제 제안서 작성 완료** |
| 2 | ~ 24.10.17(목) | **요구 분석서 작성 완료** |
| 3 | ~ 24.10.22(화) | **유즈다이어그램·프로젝트 설계서 작성** |
| 4 | ~ 24.10.25(금) | **HTML 프로토타입 / 화면설계서 작성 완료** |
| 5 | ~ 24.11.06(수) | **모든 페이지 기능 구현 완료** |
| 6 | ~ 24.11.08(금) | **검수 및 보완 수정** |
| 7 | ~ 24.11.11(월) | **배포 및 검수** |
| 8 | ~ 24.11.13(목) | **최종 발표 및 피드백** |

> **총 기간:** 2024-10-14 → 2024-11-13 
...
| **라이브러리** | - `mysql-connector-j-8.4.0.jar` (JDBC 드라이버)<br> - `cos-05Nov2002.jar` (파일 업로드)<br> - `json-simple-1.1.1.jar` (경량 JSON 처리) |
...
| 구분 | 주요 기능 | 세부 내용 |
|:--|:--|:--|
| **인증 / 세션** | 로그인 / 로그아웃 | 세션에 `loginUser` 저장 (권한, 사용자 식별자 포함) <br> 권한: A(관리자) / T(강사) / S(학생) |
| **공지사항 (관리자)** | 공지 CRUD | 목록 / 상세 / 등록 / 수정 / 삭제 <br> 관리자만 작성·수정·삭제 가능 |
| **자료실 (강사)** | 파일 업로드 / 관리 | 목록 / 상세 / 등록 / 수정 / 삭제 <br> `COS` 라이브러리 사용, 서버 `/upload` 경로 저장 |
| **Q&A 게시판** | 학생 질문 / 강사 답변 | 학생: 질문(게시글) CRUD <br> 강사: 댓글(답변) CRUD (AJAX 일부 수정 처리) |
| **강의 / 수강신청 / 출결** | 강의 및 출결 관리 | 관리자: 강의 등록·수정·삭제 <br> 학생: 강의 상세에서 수강신청 <br> 강사: 출결 체크(수기/AJAX), 학생: 인증코드 출석 |
| **마이페이지 / 관리자 페이지** | 사용자 정보 및 권한 관리 | 내 정보·연락처 수정, 수강중/종료 강의 조회 <br> 관리자: 사용자 목록 페이징 + 권한 변경(AJAX) |
